...
try {
  execSync("git add -A && git commit -m \"feat: complete Electrical AMC multi-language system\" && git push origin main", { stdio: "inherit" });
} catch (e) {
  console.log("Git main push notice:", e.message);
}

try {
  execSync("cd dist && git add -A && git commit -m \"deploy: Electrical AMC multi-language system\" && git push -f origin gh-pages && cd ..", { stdio: "inherit" });
} catch (e) {
